...
void main() {
  testWidgets(
    ...
    (t) async {
      ...
      final beforeOld = t.widgetList<Container>(
        find.ancestor(
          of: find.text('fps_max 0'),
      ...
      final oldAncestors = t.widgetList<Container>(
        find.ancestor(
          of: find.text('fps_max 0'),
          matching: find.byType(Container),
        ),
      ...
      final newAncestors = t.widgetList<Container>(
        find.ancestor(
          of: find.text('fps_max 144'),
          matching: find.byType(Container),
        ),
      );
  ...
  testWidgets('added row appears only in right column, left stays blank', (
    ...
    expect(
      find.descendant(
        of: find.byWidgetPredicate((w) => w is Container && w.color == _addBg),
  ...
  testWidgets('theme without DiffColors extension falls back to dark values', (
    ...
    expect(
      find.byWidgetPredicate(
        (w) => w is Container && w.color == _fallbackDeleteBg,
      ),
      findsOneWidget,
    );
    expect(
      find.byWidgetPredicate(
        (w) => w is Container && w.color == _fallbackAddBg,
      ),
      findsOneWidget,
    );
  });
  ...
  testWidgets('empty rows show centered no-changes hint', (t) async {
    ...
    expect(
      find.ancestor(of: find.text('No changes'), matching: find.byType(Center)),
      findsOneWidget,
    );
  });
  ...
  testWidgets('no-changes hint localizes to zh', (t) async {
    ...
    await t.pumpWidget(
      _host(SideBySideDiff(diffBloc: diff), locale: const Locale('zh')),
    );
    await t.pumpAndSettle();
